...
run_tc "TC-N-001" "Login with invalid ldapId → 401" \
  POST /auth/login \
  -H "Content-Type: application/json" \
  -d '{"ldapId":"nonexistent_user_xyz","password":"anything"}'

run_tc "TC-N-002" "Request without Authorization header → 401" \
  GET /auth/me

run_tc "TC-N-003" "Patient accessing pharmacy-only route → 403" \
  GET /prescriptions/pending \
  -H "Authorization: Bearer $PATIENT_TOKEN"

run_tc "TC-N-004" "Patient attempting to generate bill → 403" \
  POST "/visits/$NEG_VISIT_ID/bill" \
  -H "Authorization: Bearer $PATIENT_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"items\":[{\"medicineId\":\"$MEDICINE_ID\",\"quantity\":1}]}"

run_tc "TC-N-005" "Prescription with empty items array → 400" \
  POST "/visits/$NEG5_VISIT_ID/prescription" \
  -H "Authorization: Bearer $DOCTOR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"items":[]}'

run_tc "TC-N-006" "Bill with insufficient stock → 400" \
  POST "/visits/$NEG6_VISIT_ID/bill" \
  -H "Authorization: Bearer $PHARMACY_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"items\":[{\"medicineId\":\"$MEDICINE_ID\",\"quantity\":5}]}"

run_tc "TC-N-007" "Claiming a COMPLETED visit → 400" \
  PUT "/visits/$NEG_VISIT_ID/claim" \
  -H "Authorization: Bearer $DOCTOR_TOKEN"

run_tc "TC-N-008" "Duplicate prescription for same visit → 409" \
...
run_tc "TC-N-010" "QR check-in with invalid QR code → 404" \
  POST /checkin \
  -H "Authorization: Bearer $RECEPTION_TOKEN" \
  -H "Content-Type: application/json" \
