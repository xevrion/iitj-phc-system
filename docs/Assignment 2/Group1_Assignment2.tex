...
Another design question that surfaced during review was whether OCR-backed summarization should be used for scanned prescriptions or reports. The team considered this direction, especially for imported external documents, but did not include it in the implemented scope. The reason was pragmatic: the project priority was to create structured native records for prescriptions, lab requests, bills, and visit notes first. OCR would add model accuracy questions, privacy concerns, and validation burden at a stage where reliable digitization of the core workflow was still more important than summarization of legacy scans. In the current architecture, OCR remains a plausible future extension for external-document ingestion rather than a missing core module.
