...
\hline
Frontend & React + Vite; Sprint 6 UI implementation active, with login / landing, patient dashboard, and doctor dashboard already developed on the team frontend branch \\
\hline
...
The central methodology choice is to treat the visit as the anchor of all clinical artifacts. A visit progresses through states such as \texttt{WAITING}, \texttt{IN\_CONSULTATION}, \texttt{COMPLETED}, and \texttt{CANCELLED}. Once this state machine exists, prescriptions, lab requests, bills, and vitals no longer float as isolated records. The relational consequences of that choice are visible in Fig.~\ref{fig:visitid}.
...
\begin{table}[!t]
\centering
\caption{Observed Sprint Status on April 11, 2026}
\begin{tabular}{|p{0.9cm}|p{2.3cm}|p{3.0cm}|p{1.2cm}|}
\hline
\textbf{Sprint} & \textbf{Dates} & \textbf{Focus} & \textbf{Status} \\
\hline
0 & Jan 15--Jan 24, 2026 & SRS, initial UML, schema framing & Complete \\
\hline
1 & Jan 25--Feb 7, 2026 & Auth, infrastructure, patient profile & Complete \\
\hline
2 & Feb 8--Feb 21, 2026 & Visit lifecycle, doctor attendance and availability & Complete \\
\hline
3 & Feb 22--Mar 7, 2026 & Prescription and laboratory modules & Complete \\
\hline
4 & Mar 8--Mar 18, 2026 & Inventory, billing, appointments, QR check-in, documents & Complete \\
\hline
5 & Mar 19--Apr 1, 2026 & Admin users, events, reports, notifications, LDAP, integration work & Complete \\
\hline
6 & Apr 2--Apr 15, 2026 & React dashboards and UX integration & In progress \\
\hline
7 & Apr 16--Apr 20, 2026 & Hardening, testing, deployment & Not started \\
\hline
\end{tabular}
\label{tab:sprints}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Frontend integration remains the main gap:} Sprint 6 is actively underway and the login / landing page, patient dashboard, and doctor dashboard have already been implemented on the parallel frontend branch. The remaining challenge is to merge and harden that UI work against the more mature backend branch without destabilizing the documented API behavior.
...
\begin{itemize}
    ...
    \item and an active Sprint 6 frontend stream in a parallel branch, where the login / landing page, patient dashboard, and doctor dashboard are already implemented and awaiting full integration with the backend branch.
\end{itemize}
...
\hline
Frontend dashboards & Login / landing flow, patient dashboard, doctor dashboard, and broader UX integration in active Sprint 6 progress & In progress \\
\hline
\end{tabular}
\label{tab:modules}
\end{table}
...
The testing artifacts contain actual timing evidence rather than only projected estimates. These timing analyses were derived from the backend validation artifacts after Assignment 1, especially \texttt{test\_cases.csv} and the measured route-level smoke workflow. The SRS states that standard user operations should return within a few seconds under normal load. The measured results satisfy that expectation with substantial margin.
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/response_module.png}
\caption{Average measured backend response time by module from the current validation dataset.}
\label{fig:responsemodule}
\end{figure}

Fig.~\ref{fig:responsemodule} makes the module-level timing story more interpretable than raw tables alone. Authentication and patient-profile paths are the lightest operations, while prescription, laboratory, billing, and check-in flows are slower because they traverse more relations, validations, and transactional logic. Even so, the chart shows that the system remains comfortably sub-second to low-second for the main backend modules actually exercised after Assignment 1.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/threshold_observed.png}
\caption{Threshold-versus-observed timing comparison for representative backend operations.}
\label{fig:thresholdobserved}
\end{figure}

Fig.~\ref{fig:thresholdobserved} is the clearest theory-versus-implementation comparison in the current repository. The original expectation was only qualitative: key PHC operations should finish within a few seconds. The observed timings show a large headroom margin even for the heavier flows. This helped the team retain the current Node.js, Express, Prisma, and PostgreSQL stack instead of prematurely redesigning for performance reasons.

\subsection{Sprint Plan Behavior Through Burnup and Burndown}
Implementation progress is clearer when the sprint plan is plotted rather than described only in prose. The burnup and burndown charts in Fig.~\ref{fig:burnpair} are supporting project-control evidence rather than the primary technical results plots. They summarize the project trajectory across the dated sprints listed in Table~\ref{tab:sprints}. The burnup view shows that the initial release scope was slightly broader and then stabilized after early refinement, while the burndown view shows that closure accelerated during Sprint 4 and Sprint 5 once the backend architecture and module boundaries had settled.

\begin{figure}[!t]
\centering
\includegraphics[width=0.48\columnwidth]{figures/burnup.png}\hfill
\includegraphics[width=0.48\columnwidth]{figures/burndown.png}
\caption{Project burnup and burndown across the dated sprint plan.}
\label{fig:burnpair}
\end{figure}

These plots support two observations relevant to the professor's earlier feedback. First, implementation should be discussed against actual calendar dates rather than only generic week ranges. Second, the project did produce preliminary delivery results before final product completion: the backend scope was burned up meaningfully by Sprint 4, and the remaining planned work by April has shifted into Sprint 6 frontend integration and the later stabilization sprint rather than uncertain backend foundations.
...
\begin{itemize}
    ...
    \item \textbf{Frontend integration effort was underestimated.} Sprint 6 is active and core screens already exist on the team frontend branch, but integrating them cleanly with the backend, preserving role behavior, and closing polish gaps took more coordination than initially expected. This forced a clearer separation between backend completion and user-interface completion.
...
\subsubsection{OCR as a Considered but Deferred Idea}
Another design question that surfaced during review was whether OCR-backed summarization should be used for scanned prescriptions or reports. This was not part of the team's original implementation plan after Assignment 1; it emerged later as a useful recommendation from the professor. The team therefore did not design or implement an OCR pipeline in the current version. The reason for leaving it out was pragmatic: the immediate priority was to create structured native records for prescriptions, lab requests, bills, and visit notes first. OCR would add extra accuracy-validation, privacy, and model-selection burden at a stage where reliable digitization of the core PHC workflow was still more important than summarization of legacy scans. In the current architecture, OCR remains a possible future extension for imported external documents rather than a missing module that the team had already committed to build.
...
\begin{itemize}
    ...
    \item \textbf{QR-assisted reception integrated with clinical flow:} the repository implements QR-based patient resolution and a dedicated check-in path that can create a visit and initial vitals in one reception-side action instead of treating identity lookup and visit creation as separate disconnected steps.
    \item \textbf{Visit-centric outpatient traceability:} check-in, consultation, prescription, lab, billing, and documents are handled through one visit-linked model. In practice, this means the visit identifier is not just a display token; it is the relational anchor through which the main PHC artifacts are retrieved, updated, and audited.
    \item \textbf{Doctor work-queue visibility instead of only appointment listing:} the implementation supports a doctor-facing waiting queue, attendance/availability state, and doctor-specific schedule retrieval. That is more operationally useful in a PHC setting than a plain appointment calendar because walk-ins and claimed consultations are treated as first-class workflow objects.
    ...
    \item \textbf{Administrative observability and staff-state signaling:} attendance, reports, PHC events, and doctor-unavailability notifications are treated as first-class system concerns rather than afterthoughts.
\end{itemize}

At the same time, the current codebase does \emph{not} yet justify every possible implementation-level novelty claim. For example, while the schema uses primary keys and multiple unique constraints (such as \texttt{ldapId}, \texttt{qrCode}, and one-to-one \texttt{visitId} links), it does not currently define an explicit advanced indexing strategy in Prisma schema terms such as additional secondary indexes or clustered-index tuning. That is therefore not claimed here as a completed novelty point.
...
As of April 11, 2026, the clearest defensible conclusion is that the PHC Integrated Digital System has crossed the line from conceptual architecture to a functionally rich backend platform. Sprint 5 backend integration is effectively closed, Sprint 6 frontend work is actively progressing, and the remaining risks are now concentrated in frontend merge completion, integration hardening, documentation consistency, and institutional deployment details rather than in basic system design. The end-to-end path illustrated in Fig.~\ref{fig:lifecycle} is therefore no longer just aspirational architecture; it is a reasonable summary of the backend-first system that now exists, with the visible role-based interface layer approaching closure.
...
The results are mixed in the right way. The backend demonstrates substantial functional progress, working LDAP-backed development authentication, and encouraging performance behavior, while Sprint 6 has moved the frontend into visible implementation with the login / landing flow, patient dashboard, and doctor dashboard already built on the team frontend branch. At the same time, full merge readiness, final polish, and the maintenance burden of keeping testing and documentation artifacts synchronized remain real. These are not signs of conceptual failure; they are signs that the project has moved into the integration-heavy phase where disciplined engineering matters more than broad ideation.
